...
module.exports = function machineAsAction(optsOrMachineDef) {
  ...
  var action = function _requestHandler(req, res) {
    ...
    // If this is a multipart HTTP request, then check for a special header called
    // "X-JSON-MPU-Params" (could be sent by anything, but if present, it was likely
    // sent from Parasails/Cloud SDK).  If set, and clearly formatted for our use
    // case here, then we'll later attempt to JSON.parse() any of the fields that
    // are actually present and that match any of this action's known input definitions.
    // > Note that we are tolerant towards problems, since it is hypothetically
    // > possible it could be sent for some unrelated reason.
    //
    // > FUTURE: potentially move this into skipper so it works for any request,
    // > not just those handled by actions2 (would require some additional testing
    // > and consideration re: idempotency)
    var bodyParamsToReParseAsJSON = [];
    if (!req.isSocket && _.isFunction(req.is) && req.is('multipart/form-data') && _.isFunction(req.get)) {
      var rawXJSONMPUParamsHeader = req.get('X-JSON-MPU-Params');
      if (rawXJSONMPUParamsHeader && _.isString(rawXJSONMPUParamsHeader)) {
        _.each(rawXJSONMPUParamsHeader.split(','), function(paramName){
          if (paramName !== '' && _.isObject(req.body) && req.body[paramName] && _.isString(req.body[paramName])) {
            bodyParamsToReParseAsJSON.push(paramName);
          }
        });//∞
      }//ﬁ
    }//ﬁ
    ...
    var argins = _.reduce(wetMachine.getDef().inputs, function (memo, inputDef, inputCodeName) {
      ...
      // If this input corresponds witih one of the body params that need additional
      // parsing as JSON, as identified above when we parsed the "X-JSON-MPU-Params"
      // request header, then attempt that parsing now.
      // > FUTURE: if the other bit re: this header gets pulled into Skipper,
      // > then so too will this bit of code.
      else if (_.contains(bodyParamsToReParseAsJSON, inputCodeName)) {
        try {
          memo[inputCodeName] = JSON.parse(req.body[inputCodeName]);
